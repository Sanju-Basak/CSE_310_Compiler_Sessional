...
public:
    SymbolInfo* next;
    string temp= "";    //for array size name
    int bpOff= -1;
    string forLabel1;
    string forLabel2;
    string forLabel3;
    ...
    int getParamIndex(string str){
        for(int i=0; i<param_list.size(); i++){
            if(str== param_list[i].par_name)
                return i;
        }
        return -1;
    }
...
std::pair<string, SymbolInfo*> lookUpAll2(string str)
{
    ScopeTable *temp;
    SymbolInfo* smb;
    temp= current;
    while(temp != NULL){
        ...
        if(smb != NULL)
        {
            return std::make_pair(temp->id, smb);
        }
        temp= temp->parentScope;
    }
   //cout<< "Not found\n";
    //return std::make_pair(" ", NULL);
}
